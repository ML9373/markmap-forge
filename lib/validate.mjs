...
function checkViews(views, markdown) {
	const diagnostics = [];
	if (!views || !views.length) return diagnostics;
	const haystack = markdown.toLowerCase();
	const seenIds = new Set();
	for (const view of views) {
		if (seenIds.has(view.id)) {
			diagnostics.push(diag("views/duplicate-id", "error", `view id "${view.id}" is used more than once.`));
		}
		seenIds.add(view.id);
		for (const term of view.focus) {
			if (!haystack.includes(term.toLowerCase())) {
				diagnostics.push(
					diag(
						"views/ungrounded-focus",
						"error",
						`view "${view.id}": focus term "${term}" does not appear anywhere in the markdown — a guided tour stop must be grounded in real content, not invented.`
					)
				);
			}
		}
...
export function validateMindmap(data) {
	...
	diagnostics.push(...checkViews(data.views, data.markdown));
