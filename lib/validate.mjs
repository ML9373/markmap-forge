...
/**
 * Ajv is resolved from node_modules when the package was installed normally, and falls back
 * to the vendored single-file bundle otherwise.
 *
 * The fallback exists because this repo is also deployed as a vault skill, where the agent
 * must be able to run the CLI with no prior `npm install`. Shipping node_modules into that
 * copy is not an option: it drags ~2.7 MB and ~60 auto-generated markdown folder notes into
 * the vault, which then surface in every vault-wide MOC and broken-link scan.
 *
 * Regenerate the bundle after bumping ajv with:
 *   npx esbuild node_modules/ajv/dist/ajv.js --bundle --format=esm --platform=node \
 *     --outfile=vendor/ajv.bundle.mjs --legal-comments=inline
 */
async function loadAjv() {
	try {
		return (await import("ajv")).default;
	} catch {
		return (await import("../vendor/ajv.bundle.mjs")).default;
	}
}
