...
description: Convert a technical document (specs, slides, PDFs, markdown) into ONE standalone interactive HTML5 Strategic Mind Map, exhaustive or light. Use when the user wants a document turned into an explorable mind map with search, dark mode, minimap, pitch mode, and SVG export instead of a slide deck.
license: MIT
metadata:
  version: "0.1"
...
3. Choose `variant`:
   - `exhaustive` — a complete knowledge model: every definition, dependency, association, constraint, warning, and value list present in the source.
   - `light` — critical-only: the dependency chain, high-leverage constructs, and operational rules; omit long enumerations, keep at most representative examples.
4. Write the candidate JSON matching `schemas/mindmap.schema.json`:
   ```json
   {
     "schema_version": 1,
...
6. Deliver once validation passes:
   ```bash
...
7. Open the delivered HTML and sanity-check it (see `examples/example.html` for a working reference) before handing it back.
...
Authored per document: `source_name`, `variant`, `generation_date`, and the sanitized `markdown` outline. That's the entire surface area — resist the temptation to touch `templates/strategic-map.template.html` for a one-off document; if the template itself needs a new feature, that's a renderer change, reviewed and tested once, not a per-document improvisation.

## Multiple documents, one theme

For a set of related documents (e.g. one per industry vertical, or one per subsystem), render each to its own `<title>_<variant>.html` — do not try to merge unrelated sources into one mind map; that breaks the "strictly source-based" and MECE rules from step 2.
